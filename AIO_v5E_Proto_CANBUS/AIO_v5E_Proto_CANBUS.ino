...
- ISO GPS position can be send




 */
bool sendGPStoISOBUS_129029 = false;
bool sendGPStoISOBUS_65267_65256 = false;
bool sendGPStoISOBUS_65254 = false;
 //----------------------------------------------------------
...
String inoVersion = ("\r\nAgOpenGPS CANBUS Ver 2025.09.04 (AIO v5 Proto PCB))");
...
int16_t WorkSWAnanlogThres = 2000; // no const, to set threshold externaly
...
uint8_t Brand = 0;// 1;              //Variable to set brand via serial monitor.
uint8_t gpsMode = 2;// 1;            //Variable to set GPS mode via serial monitor.
...
uint8_t steeringValveReady = 8;   //Variable for Steering Valve State from CAN
uint8_t previousValveState = 8;
...
struct Setup
{
	...
	uint8_t IsDanfoss = 0;
	uint8_t IsUseY_Axis = 0;// 1;     //0 = use X Axis, 1 = use Y axis
...
void setup()
{
	...
	Serial.print("\nESP32 gateway started at "); Serial.print(baudESP32); Serial.println(" baud. Settings at 192.168.137.1\n");
#endif
	...
	if (EEread != EEP_Ident)   // check on first start and write EEPROM
	{
		...
		EEPROM.put(74, sendGPStoISOBUS_129029);
		EEPROM.put(76, sendGPStoISOBUS_65267_65256);
		EEPROM.put(78, sendGPStoISOBUS_65254);
	}
	else
	{
		...
		sendGPStoISOBUS_129029 = EEPROM.read(74);
		sendGPStoISOBUS_65267_65256 = EEPROM.read(76);
 		sendGPStoISOBUS_65254 = EEPROM.read(78);
	}
  ...
  //----Teensy 4.1 CANBus--End---------------------
	...
	Serial.println("\r\nFor Debug Info WheelAngleSensor use CANBUS Service Tool\n");
	Serial.println("\r\nFor Debug Info Enter: 'e': ESP32, 'w': workswitch");
...
void loop()
{
	...
	if (currentTime - lastTime >= LOOP_TIME)
	{
		...
		LEDs.updateLoop(); // LEDS.h V5 Proto
		...
		//Workswitch reading
		uint16_t read = analogRead(ANAKO_PIN); // read work input, the height sensor
		currentReading = map(read, 1, 1024, 1, 255);

		if (workDebug)
		{
			Serial.print("AnaKO Teensy ADC: ");
			Serial.print(read);
		}

		if(digitalRead(WORKSW_PIN)) //HIGH mean Active here
		{
			if (steerConfig.CurrentSensor && currentReading > steerConfig.PulseCountMax) //Set to check for height sensor, too high
			{
				read = HIGH; //not active	
			}
			else 
			{
				read = LOW; //active, workswitch is on and heigt sensor is either low or desactivated
			}
		}
		else
		{
			read = HIGH; //not active, no workswich, wathever heigh positon		
		}
		...
		if (read != workSwitch)
		{
			if (workDebug) Serial.printf("\r\nWORK input: %s", (read == 1 ? "OFF" : "ON"));
			workSwitch = read;
		}

		// set via CAN?????? no idea, just copy pasted it
		if (steerConfig.PressureSensor)  workSwitch = !workCAN;         // If CAN workswitch is on, set workSwitch ON
		...
		if (reading == LOW && previous == HIGH)
		{
			if (currentState == 1)
			{
				...
				if(steeringValveReady != 20 && steeringValveReady != 16)
				{
					showCanbusStateOnAOG(steeringValveReady);
				}
			}
			else
			{
		...
		if (steeringValveReady != 20 && steeringValveReady != 16)
		{
			...
			if(steeringValveReady != previousValveState) showCanbusStateOnAOG(steeringValveReady);

			previousValveState = steeringValveReady;
		}
		...
		{
			if (WASDebug) { Serial.print("setCurve: "); Serial.print(setCurve); }
			...
			if (WASDebug) { Serial.print(" steeringPosition: "); Serial.print(steeringPosition); }
			...
			if (WASDebug) { Serial.print(" steerAngleActual: "); Serial.print(steerAngleActual); }
			...
			if (WASDebug) { Serial.print(" mappedWAS: "); Serial.println(mappedWAS); }
	...
	if (SerialESP32.available())// while (SerialESP32.available())
	{
		...
		if (ESP32gotLFCR)
		{
			...
			Udp.write(ESP32incomingBytes, ESP32InPacketLength);
			Udp.endPacket();
			//	}
			ESP32gotLFCR = false;
...
void udpSteerRecv(int sizeToRead)
{
...
#ifdef isAllInOneBoard
	SerialESP32.write(udpData, sizeToRead);
#endif
	...
	if (udpData[0] == 0x80 && udpData[1] == 0x81 && udpData[2] == 0x7F) //Data
	{
		...
		else if (udpData[3] == 0xFB)  //251 FB - SteerConfig
		{
			...
			Serial.print("got steer settings, useY Axis: "); Serial.println(steerConfig.IsUseY_Axis);
			//crc
			//udpData[13];        
		...
		else if (udpData[3] == 0xD0)  //Corrected GPS Data
		{
			...
			if (sendGPStoISOBUS_65267_65256)
			{
				if (GPS_5hz > 4)
				{
					GPS_5hz = 0;
