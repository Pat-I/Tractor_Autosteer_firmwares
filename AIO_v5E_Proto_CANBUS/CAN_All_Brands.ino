    ...
    if (V_Bus.read(VBusReceiveData)) {
        ...
        if (Brand == 0)
        {
          ...
          if (VBusReceiveData.id == 0x18EF1CD2)
          {
            ...
            else if ((VBusReceiveData.buf[1]) == 1) //MTZ8302 for Axion 810 2021
            {
                CAN_Engage();
                engageCAN = bitRead(VBusReceiveData.buf[0], 2);
                //*****Turn saftey valve ON**********
...
void showCanbusStateOnAOG( uint8_t state ){
  switch( state ){
    case 0x00: { //dec 00
      ...
      SendHardwareMessage(data, sizeof( data ));
    }
    break;
    case 0x20: { // dec 32
      uint8_t data[26] = {0x80, 0x81, 0x7F, 0xDD, 20, 3, 1, 0x54, 0x75, 0x72, 0x6E, 0x20, 0x73, 0x74, 0x65, 0x65, 0x72, 0x69, 0x6E, 0x67, 0x77, 0x68, 0x65, 0x65, 0x6C}; //Turn steeringwheel
      int CRCtoAOG = 0;
      for ( byte i = 2; i < sizeof( data ) - 1; i++ ){
        CRCtoAOG = ( CRCtoAOG + data[ i ] );
      }
    ...
    break;
    case 0x08: {
      uint8_t data[17] = {0x80, 0x81, 0x7F, 0xDD, 11, 8, 0, 0x4E, 0x6F, 0x20, 0x43, 0x41, 0x4E, 0x42, 0x55, 0x53}; //No CANBUS
      int CRCtoAOG = 0;
      for ( byte i = 2; i < sizeof( data ) - 1; i++ ){
        CRCtoAOG = ( CRCtoAOG + data[ i ] );
      }
    ...
    break;
    default: {
      uint8_t data[21] = {0x80, 0x81, 0x7F, 0xDD, 15, 3, 1, 0x55, 0x6E, 0x6B, 0x6E, 0x6F, 0x77, 0x6E, 0x20, 0x65, 0x72, 0x72, 0x6F, 0x72}; //Unknown error
      int CRCtoAOG = 0;
      for ( byte i = 2; i < sizeof( data ) - 1; i++ ){
        CRCtoAOG = ( CRCtoAOG + data[ i ] );
      }
      data[ sizeof( data ) - 1 ] = CRCtoAOG;
      SendHardwareMessage(data, sizeof( data ));
      Serial.println(steeringValveReady);
    }
    break;
  }
}

void SendHardwareMessage(uint8_t data[], uint size){
