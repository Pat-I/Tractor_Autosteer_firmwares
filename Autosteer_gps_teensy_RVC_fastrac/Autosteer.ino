/*
   ...
   01 Feb 2022
   modified by Pat
   with implemeent output for iso plug
  PWM frequency 122hz
  IMP_SW on 27 (A13)

  27 feb 2024 -added WAS safety line 444
  changed autosteer loop to 50 hz
...
//Steer switch button  ***********************************************************************************************************
uint8_t currentState = 1, reading = 1, readingWork = 1, readingWorkTwo = 0, previous = 0;
...
void autosteerSetup() {
  ...
  */
  ...
  // load networkAddress factory/firmware defaults
  //set
...
void autosteerLoop() {
  ...
  // Loop triggers every 20 msec and sends back gyro heading, and roll, steer angle etc
  ...
  if (currentTime - autsteerLastTime >= LOOP_TIME) {
    ...
    */
    ...
    //Check if WAS value is usable
    if (abs(helloSteerPosition) > 1925)  //raw value <123(0.15v) or >3973(4.85v)
    {

      watchdogTimer = WATCHDOG_FORCE_VALUE;  //turn off steering motor imediatly
      steerSwitch = 1;                       // reset values like it turned off
      currentState = 1;
      previous = 0;
    }
    ...
    //Serial.print("\rAS loop time us: "); Serial.println(micros() - autsteerStartTimeuS);
    //Serial.print("\rAS loop time ms: "); Serial.println(systick_millis_count - autsteerLastTime);
  }  //end of timed loop
